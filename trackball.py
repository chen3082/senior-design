...
class Trackball(object):
    ...
    def __repr__(self):
        rot = '[%.3f, %.3f, %.3f, %.3f]' % tuple(self._rotation)
        trans = '[%.3f, %.3f, %.3f]' % tuple(self._trans)
        return 'Trackball(rot=%s, \ntrans=%s)' % (rot, trans)
        # phi = '%.1f' % self.phi
        # theta = '%.1f' % self.theta
        # zoom = '%.1f' % self.zoom
        # rot = '[%.2f, %.2f, %.2f, %.2f]' % tuple(self._rotation)
        # trans = '[%.2f, %.2f, %.2f]' % tuple(self._trans)

...
import click
import time
from flappy.envs.fwmav.window import GLUTWindow
from flappy.envs.fwmav.trackball import Trackball
...
from flappy.envs.fwmav.controllers.controller_maneuver import PIDController
...
class Simulation:
    ...
    def __init__(self, mav_config_list, sim_config):
        ...
        self.flapper1 = FWMAV(mav_config_list[6],
                              self.world)  # 2 is latest trim with base, 4 is without base, 6 is with small base
    ...
    def get_reward(self, action):
        ...
        normalized_action[0] = (action[0] - self.total_action_lb[0]) / (
                    self.total_action_ub[0] - self.total_action_lb[0])
        ...
        cost = (
                           control_cost + d_control_cost + stability_cost) * 3  # 5 is tight, good for position control, 3 is looser
    ...
    def step(self, action):
        ...
        input_voltage[0] = self.generate_control_signal(34, max_voltage, voltage_diff, voltage_bias, -split_cycle,
                                                        self.world.time(), 0)
        input_voltage[1] = self.generate_control_signal(34, max_voltage, -voltage_diff, voltage_bias, split_cycle,
                                                        self.world.time(), 0)
        ...
        right_CoP = np.array(
            [0, (-1) * self.flapper1.right_wing.GetSpanCoP(), (-1) * self.flapper1.right_wing.GetChordCoP()])
    ...
    def init_world(self):
        ...
        # world = dart.simulation.World()  jeff
    ...
    def update_camera(self):
        self.glutwindow.scene.replace_camera(0, Trackball(theta=self.camera_theta, phi=self.camera_phi,
                                                          trans=[self.camera_horizontal, self.camera_vertical,
                                                                 self.camera_depth]))
